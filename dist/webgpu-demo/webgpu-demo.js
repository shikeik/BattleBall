...
class WebGPUDemo {
    ...
    async init() {
        ...
        if (!navigator.gpu) {
            if (window.logger)
                logger.log('WEBGPU', 'WebGPU not supported on this browser');
        ...
        if (!adapter) {
            if (window.logger)
                logger.log('WEBGPU', 'No appropriate GPUAdapter found');
        ...
        if (window.logger)
            logger.log('WEBGPU', 'WebGPU demo initialized successfully');
        return true;
    }
    async setupBuffers() {
    ...
    start() {
        ...
        loop();
        if (window.logger)
            logger.log('WEBGPU', 'Demo started');
    }
    stop() {
        if (this.animationId) {
        ...
        if (window.logger)
            logger.log('WEBGPU', 'Demo stopped');
    }
    destroy() {
        this.stop();
        // Cleanup resources
...
//# sourceMappingURL=webgpu-demo.js.map