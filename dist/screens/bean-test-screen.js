...
class BeanTestScreen extends Screen {
    ...
    async enter() {
        ...
        if (!window.BeanManager) {
            if (window.logger)
                logger.log('BEAN_TEST', 'BeanManager not found');
            return;
        }
        ...
        if (!success) {
            if (window.logger)
                logger.log('BEAN_TEST', 'WebGPU init failed');
            return;
        }
        // 生成彩豆
        ...
        if (!canvas) {
            if (window.logger)
                logger.log('BEAN_TEST', 'Canvas not found');
            return;
        }
        // 配置 WebGPU context
        ...
        if (!context) {
            if (window.logger)
                logger.log('BEAN_TEST', 'WebGPU context not available');
            return;
        }
        ...
        if (window.logger)
            logger.log('BEAN_TEST', 'Initialized successfully');
        // 启动渲染循环
    ...
    _render() {
        if (!this.beanManager || !this.beanManager.device)
            return;
        const canvas = document.getElementById('gameCanvas');
...
//# sourceMappingURL=bean-test-screen.js.map