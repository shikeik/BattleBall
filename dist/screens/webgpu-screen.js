...
class WebGPUScreen extends Screen {
    ...
    async _initWebGPU() {
        if (this.initializing || this.webgpuDemo)
            return;
        ...
        try {
            ...
            if (window.logger)
                logger.log('WEBGPU_SCREEN', 'WebGPU demo started');
        }
        catch (e) {
            this.error = e.message;
            if (window.logger)
                logger.log('WEBGPU_SCREEN', 'Failed to init', { error: e.message });
            // 清理
            if (this.webgpuCanvas) {
                this.webgpuCanvas.remove();
                this.webgpuCanvas = null;
...
//# sourceMappingURL=webgpu-screen.js.map