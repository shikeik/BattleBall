...
class BeanManager {
    ...
    async _createPipeline() {
        ...
        if (window.logger)
            logger.log('BEAN', 'Render pipeline created');
    }
    generateBeans() {
    ...
    _createInstanceBuffer() {
        try {
            ...
            if (window.logger)
                logger.log('BEAN', `Creating instance buffer: ${bufferSize} bytes`);
            ...
            if (window.logger)
                logger.log('BEAN', 'Instance buffer created successfully');
        }
        catch (e) {
            if (window.logger)
                logger.log('BEAN', `Failed to create instance buffer: ${e.message}`);
            throw e;
        }
    }
