...
class LoadingScreen {
	...
	private initTasks: Array<{ name: string, task: () => Promise<void>, weight: number }>;
	...
	create() {
		...
		this.progressBar = document.getElementById('loading-progress') as HTMLDivElement;
		this.progressText = document.getElementById('loading-percent') as HTMLDivElement;
	 ...
	 */
	registerTask(name: string, task: () => Promise<void>, weight = 1) {
	...
	async start() {
		...
		for (let i = 0; i < this.initTasks.length; i++) {
			...
			try {
				...
				if (window.logger) window.logger.log('LOADING', `Task failed: ${name}`, { error: (e as Error).message });
			}
