...
class Joystick {
	...
	private dx: number;
	private dy: number;
	private onMove: ((dx: number, dy: number) => void) | null;
	...
	constructor(options: any = {}) {
		...
		if (window.logger) window.logger.log('JOYSTICK', 'Joystick created');
	}
	 ...
	 * 获取摇杆中心位置（屏幕坐标）
	 */
	_getCenter(screenWidth: number, screenHeight: number): { x: number, y: number } {
		return {
			x: this.position.x,
