...
class BeanManager {
	...
	async initRenderResources(canvas) {
		if (!this.device) {
			if (window.logger) logger.log('BEAN_ERROR', 'initRenderResources: no device');
			return false;
		}
		
		this.context = canvas.getContext('webgpu');
		if (!this.context) {
			if (window.logger) logger.log('BEAN_ERROR', 'initRenderResources: no webgpu context');
			return false;
		}
		...
		if (window.logger) logger.log('BEAN', 'initRenderResources success');
		return true;
	}
	...
	render(cameraX, cameraY, zoom, screenWidth, screenHeight) {
		// 错误检查
		if (!this.device) { 
			if (window.logger) logger.log('BEAN_ERROR', 'no device'); 
			return; 
		}
		if (!this.pipeline) { 
			if (window.logger) logger.log('BEAN_ERROR', 'no pipeline'); 
			return; 
		}
		if (!this.context) { 
			if (window.logger) logger.log('BEAN_ERROR', 'no context'); 
			return; 
		}
		if (!this.instanceBuffer) { 
			if (window.logger) logger.log('BEAN_ERROR', 'no instanceBuffer'); 
			return; 
		}
		if (!this.bindGroup) { 
			if (window.logger) logger.log('BEAN_ERROR', 'no bindGroup'); 
			return; 
		}
		
		if (window.logger) logger.log('BEAN', `render: ${this.beanCount} beans`);
